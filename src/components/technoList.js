...
import { fetchTechnology } from '../actions'
...
class TechnoList extends React.Component {
    componentDidMount() {
                        ...
                        <div className="col-lg-3 " key={value.id = uuidv4()}>
                            ...
                            <div className="embed-responsive embed-responsive-16by9">
                                ...
                                <div className="card-image">
                                        ...
                                        <a className="card-action-techno card-body" ><img src="https://cdn-icons-png.flaticon.com/512/2107/2107845.png" height={25} /></a>
                                        <div className="card-heading card-body ">
                                            {truncateOverview(value.title, 50)}
                                        </div>
                                        <div className="card-text card-body">
                                            {truncateOverview(value.description, 75)}
                                        </div>
                                        <div className="card-text" style={{color : "#8c10f1e1"}}>
                                    ...
                                    {value.publishedAt.substr(7,10).slice(1,-7)}.{value.publishedAt.substr(5,8).slice(0,-6)}.{value.publishedAt.substr(0,5).slice(0,-1)} 
                                        ...
                                        <a href={value.url} className="card-button-techno" target="_blank" rel='noreferrer'> Show More</a>
                                    </div>
                                </div>
                                </div>
                            </div>
...
const mapStateToProps = (state) => {
    return { techno: state.techno }
}

export default connect(mapStateToProps, { fetchTechnology })(TechnoList);